...
#else
extern CommonUniverse::CWebRTImpl* g_pWebRTImpl;
#endif
        ...
        if (strVer != _T("")) {
          strPath = strBase + strVer + _T("\\") + installer::kChromeDll;
            *module = base::FilePath(strPath.GetBuffer());
            ...
            if (!dll)
              PLOG(ERROR) << "Failed to load Chrome DLL from "
                          << module->value();
            return dll;
        }
    }
...
int MainDllLoader::Launch(HINSTANCE instance,
  ...
  if (g_pWebRTImpl && g_pWebRTImpl->m_pBrowserFactory) {
      delete g_pWebRTImpl->m_pBrowserFactory;
      g_pWebRTImpl->m_pBrowserFactory = nullptr;
  }
  // end Add by TangramTeam 
  return rc;
}
