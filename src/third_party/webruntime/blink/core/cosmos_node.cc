...
CosmosNode* CosmosNode::getChild(const String& strName) {
  auto it = m_mapChildNode2.find(Cosmos::S2w(strName));
...
CosmosNode* CosmosNode::getXobj(const String& strName) {
  ...
  auto it = root_->m_mapXobj.find(Cosmos::S2w(strName));
...
//m_pVisibleContentElement = contentElement; 							break;
//						}
//					}
//					else
//					{
...
// exception_state); 			m_pVisibleContentElement->classList().remove({ "show" },
// exception_state); 			m_pVisibleContentElement->classList().add({
// "hidden" },
// exception_state); 			m_pVisibleContentElement = visibleElem2;
//		}
//	}
