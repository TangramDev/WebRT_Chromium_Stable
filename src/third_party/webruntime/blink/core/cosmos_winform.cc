...
CosmosGalaxy* CosmosWinform::getGalaxy(const String& galaxyName) {
  auto it = m_mapWebRTGalaxy.find(Cosmos::S2w(galaxyName));
...
CosmosNode* CosmosWinform::getXobj(const String& galaxyName,
  ...
  if (it != m_mapWebRTGalaxy.end()) {
    ...
    auto it2 = it->second->m_mapWebRTNode2.find(
        Cosmos::S2w(clusterName_));
  ...
  if (elem) {
    ...
    if (pNode->getNodeType() == 1) {
      ...
      if (target.IsNull() == true || target == "") {
        auto it = m_mapWebRTGalaxy.find(Cosmos::S2w(galaxy));
        if (it != m_mapWebRTGalaxy.end()) {
          auto it2 = it->second->m_mapRootNode.find(
              Cosmos::S2w(winNucleus));
  ...
  if (strMessageXml.IsNull() == false && strMessageXml != "") {
    ...
    if (DocumentFragment_) {
      ...
      if (DocumentFragment_->Children()->length()) {
        ...
        m_strAppContents = hostElem_->getAttribute("formdiv");
