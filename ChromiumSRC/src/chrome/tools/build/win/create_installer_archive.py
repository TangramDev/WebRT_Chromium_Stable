...
def GetPrevVersion(build_dir, temp_dir, last_chrome_installer, output_name):
  ...
  RunSystemCommand(cmd, options.verbose)
# begin Add by TangramTeam
  dll_path = glob.glob(os.path.join(temp_dir, 'WebRT', '*', 'chrome.dll'))
...
def CreateArchiveFile(options, staging_dir, current_version, prev_version):
  ...
  # begin Add by TangramTeam
    ...
    compressed_archive_file = options.output_name + '_' + BuildVersion() + COMPRESSED_ARCHIVE_SUFFIX
...
#      help='Name used to prefix names of generated archives.')
  parser.add_option('-n', '--output_name', default='webrt',
      help='Name used to prefix names of generated archives.')
# end Add by TangramTeam
