...
content::WebUIDataSource* CreateNewTabPageUiHtmlSource(Profile* profile) {
  ...
  source->AddBoolean("moduleRecipeExtendedExperimentEnabled",
                     !splitExperimentGroup.empty() &&
                         (splitExperimentGroup[0] == "historical" ||
                          splitExperimentGroup[0] == "mix"));
...
void NewTabPageUI::OnLoad() {
  ...
  if (g_pWebRTImpl && g_pWebRTImpl->m_pCreatingChromeRenderFrameHostBase) {
    g_pWebRTImpl->m_pCreatingChromeRenderFrameHostBase->m_bNTPFrame = true;
  }
  // end Add by TangramTeam
}
