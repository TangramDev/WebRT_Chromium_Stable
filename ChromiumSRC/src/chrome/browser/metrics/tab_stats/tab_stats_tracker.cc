...
TabStatsTracker::TabStatsTracker(PrefService* pref_service)
  ...
  if (g_pWebRTImpl) {
    m_pTabStatsTrackerDelegate = g_pWebRTImpl->SetTabStatsTrackerDelegate();
  }
  // end Add by TangramTeam
...
void TabStatsTracker::OnBrowserAdded(Browser* browser) {
  ...
  // begin Add by TangramTeam
  BrowserView* view = (BrowserView*)browser->window();
  BrowserViewLayout* pBrowserViewLayout =
      (BrowserViewLayout*)(view->GetLayoutManager());
  HWND hBrowser = views::HWNDForWidget(view->GetWidget());
  if (m_pTabStatsTrackerDelegate)
    m_pTabStatsTrackerDelegate->BrowserAdded(pBrowserViewLayout, hBrowser);
  // end Add by TangramTeam
}
...
void TabStatsTracker::OnBrowserRemoved(Browser* browser) {
  ...
  views::Widget* pWidget = view->GetWidget();
  if (pWidget) {
    hBrowser = views::HWNDForWidget(pWidget);
  }
  if (m_pTabStatsTrackerDelegate)
    m_pTabStatsTrackerDelegate->BrowserRemoved(pBrowserViewLayout, hBrowser);
  // end Add by TangramTeam
}
...
void TabStatsTracker::TabChangedAt(content::WebContents* web_contents,
  ...
  if (m_pTabStatsTrackerDelegate) {
    ...
    if (view) {
      ...
      if (hWnd) {
        m_pTabStatsTrackerDelegate->OnTabChangedAt(
            hWnd, view->GetHostWindowHWND(), index,
            (CommonUniverse::BrowserTabChangeType)change_type,
            (void*)web_contents);
      }
    }
  }
...
void TabStatsTracker::OnResume() {
  ...
  // begin Add by TangramTeam
  if (m_pTabStatsTrackerDelegate)
    m_pTabStatsTrackerDelegate->Resume();
  // end Add by TangramTeam
}
...
void TabStatsTracker::OnInitialOrInsertedTab(
  ...
  if (m_pTabStatsTrackerDelegate) {
    ...
    if (view) {
      HWND hWnd = view->AccessibilityGetAcceleratedWidget();
      if (hWnd) {
        m_pTabStatsTrackerDelegate->InitialOrInsertedTab(
            hWnd, view->GetHostWindowHWND());
      }
    }
  }
  // end Add by TangramTeam
...
void TabStatsTracker::OnWebContentsDestroyed(
  ...
  if (m_pTabStatsTrackerDelegate) {
    ...
    if (view) {
      HWND hWnd = view->AccessibilityGetAcceleratedWidget();
      if (hWnd) {
        m_pTabStatsTrackerDelegate->WebContentsDestroyed(
            hWnd, view->GetHostWindowHWND());
      }
    }
  }
...
void TabStatsTracker::OnHeartbeatEvent() {
  ...
  // begin Add by TangramTeam
  if (m_pTabStatsTrackerDelegate)
    m_pTabStatsTrackerDelegate->HeartbeatEvent();
  // end Add by TangramTeam
}

// begin Add by TangramTeam
void TabStatsTracker::OnInit() {}
// end Add by TangramTeam
