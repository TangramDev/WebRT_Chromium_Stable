...
int AppMessagePumpForUI::OnProcessNextWindowsMessage(LPMSG lpMsg) {
  return -1;
}
  ...
  if (g_pWebRTImpl == nullptr) {
    ...
    if (hModule != nullptr) {
      ...
      if (GetWebRTImplFunction != NULL) {
        ...
        g_pWebRTImpl = GetWebRTImplFunction(&pCosmos);
      }
    }
  }
  if (g_pWebRTImpl &&
      g_pWebRTImpl->m_dwThreadID == ::GetCurrentThreadId()) {
    m_pWebRTDelegate = g_pWebRTImpl->m_pWebRTDelegate;
    is_ui_MessagePump_ = true;
  }
  // end Add by TangramTeam
}

// begin Add by TangramTeam
int MessagePumpForUI::OnProcessNextWindowsMessage(LPMSG lpMsg) {
  DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
  ...
  more_work_is_plausible |= ProcessMessageHelper(*lpMsg);
  ...
  if (!more_work_is_plausible) {
    ...
    // DoIdleWork() shouldn't end up in native nested loops and thus
    // shouldn't have any chance of reinstalling a native timer.
    ...
    if (!more_work_is_plausible) {
      if (g_pWebRTImpl->m_nAppType != APP_BROWSER &&
          g_pWebRTImpl->m_nAppType != APP_ECLIPSE &&
          g_pWebRTImpl->m_nAppType != APP_BROWSER_ECLIPSE &&
          g_pWebRTImpl->m_hMainWnd == 0) {
        run_state_->should_quit = true;
        return 1000;
      }

      if (m_pWebRTDelegate->IsAppIdleMessage()) {
        bIdle = TRUE;
        lIdleCount = 0;
      }
      if (!m_pWebRTDelegate->DoIdleWork())
...
MessagePumpForUI::~MessagePumpForUI() {
  if (g_pWebRTImpl) {
    g_pWebRTImpl->OnExitMainMsgPump();
  }
}
// end Add by TangramTeam
...
void MessagePumpForUI::DoRunLoop() {
  ...
  // begin Add by TangramTeam
  if (m_pWebRTDelegate) {
    g_pWebRTImpl->m_pMessagePumpForUI =
        (CommonUniverse::CosmosAppMessagePumpForUI*)((
            AppMessagePumpForUI*)this);
    g_pWebRTImpl->ConnectMsgLoop(message_window_.hwnd());
    if (g_pWebRTImpl->m_bHostMsgLoop)
      return;
    m_bStartRun = true;
    for (;;) {
      ...
      // m_pWebRTDelegate->OnAppIdle(bIdle, lIdleCount);
