...
void CosmosXobj::sendMessage(CosmosXobj* msg, V8ApplicationCallback* callback) {
  if (m_pRenderframeImpl) {
    ...
    if (callback) {
      ...
      m_pRenderframeImpl
          ->m_mapWebRTSession[Cosmos::S2w(callbackid_)] = this;
    }
    m_pRenderframeImpl->SendCosmosMessageEx(msg->session_);
    msg->setStr("msgID", "");
  }
...
void CosmosXobj::invokeCallback(wstring callbackid, CosmosXobj* callbackParam) {
  auto itcallback =
      mapWebRTEventCallback_.find(Cosmos::w2S(callbackid));
  if (itcallback != mapWebRTEventCallback_.end()) {
    blink::V8ApplicationCallback* callback =
        (blink::V8ApplicationCallback*)itcallback->value.Get();
        ...
        for (int i = 0; i < nCount; i++) {
          ...
          if (nHandle) {
            ...
            if (it != cosmos_->m_mapWebRTNode.end()) {
              ...
              auto it1 =
                  m_mapElement.find(_strID + L"__" + std::to_wstring(nHandle));
              if (it1 != m_mapElement.end())
                elem = it1->second;
              xobjfortarget = it->value;
              if (!!xobjfortarget) {
...
void CosmosXobj::DispatchXobjEvent(Element* e, const String& eventName) {
  ...
  //           var winNucleus = elem.getAttribute("nucleus");
  //            if (!!target && !!galaxy && !!winNucleus) {
  //                var xobjfortarget = apppage.getXobj(galaxy, winNucleus,
  //                target); if (!!xobjfortarget) {
...
NodeList* CosmosXobj::getElementsByName(const String& localName) {
